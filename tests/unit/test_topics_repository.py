"""Unit tests for app.domain.topics.repositories.TopicRepository."""
...
from app.domain.topics.repositories import TopicRepository, _use_fts
...
class TestTopicRepository:
    ...
    async def test_list_topics_cursor_with_results(self):
        ...
        rows, prev_id, has_more, next_id = await repo.list_topics_cursor(page_size=10)
        assert rows == [t1, t2]
        assert has_more is False
        assert next_id is None
    ...
    async def test_list_topics_cursor_has_more(self):
        ...
        rows, prev_id, has_more, next_id = await repo.list_topics_cursor(page_size=2)
        assert len(rows) == 2
