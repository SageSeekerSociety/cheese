...
| Session / work lease | Chat starts without renting hands; the first execution tool acquires its own session lease. Check separate agents, owner authorization, stale generations, explicit unknown confirmation, background work and unreachable-machine approval before replacement, and retained-resource cleanup. | `backend/tests/integration/test_turn_without_files_rents_no_hands.py`; `backend/tests/integration/test_session_work_leases.py`; `backend/tests/contract/test_dispatch_record_contract.py`; `backend/tests/integration/test_archive_retires_storage.py` |
...
| Project export / offline files | Human membership and private-room scope; offline Git clone, original file names and checksums; missing artifacts, corrupt chunks and interrupted download cleanup. | `backend/tests/integration/test_project_export.py` |
