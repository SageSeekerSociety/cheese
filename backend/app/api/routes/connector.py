...
from app.agent.sessions import Session, SessionStore
from app.agent.wiring import ConnectorGraph, build_default_graph
...
# App-wide default graph + router, so ``app.main`` mounts this exactly like
# every other route module (``app.include_router(connector.router)``). Tests
# and the scratchpad demo inject their own graph via
# ``build_router(build_default_graph(...))``; this is only the process default.
graph = build_default_graph()
router = build_router(graph)
