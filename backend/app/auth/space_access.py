"""谁是一个题目板的管理员（= 教师），以及这条主张能开哪几扇门 —— 一处。

产品把定义拍死了：**题目板（space）= 一门课 / 一个活动**，**教师就是这个题目板的
管理员**。落到库里，教师就是 ``space_admin_relation`` 里的一行 —— 建版的人
（``SpaceService.create_space`` 写进去的 OWNER）与创建者后来设的 ADMIN。这条规则
本身早就在 ``SpaceRepository.build_membership_predicate`` 里写过一遍（成员行 OR
管理员关系）；本模块是它的**单点形态**，给「这人是不是这个板子的教师」这个是非题
用，而不是列表谓词。

为什么单独一个模块、而不是继续往各处路由里写 ``task.creator_id != user_id``：
那条判据是「这道题的出题者」，比「这门的教师」窄。一门课里有几十道题、几位助教，
...
async def is_admin_of_projects_task(
    ...
    """这个项目的赛题所挂题目板的教师 —— ``may_read_project`` 用的第五条主张。

    ``project.external_task_id`` → ``Task.space_id`` → 管理员关系。题不存在、没挂
    板、或人不是管理员都是 False。取的就是「教师版面」，比出题者那条宽一格：一门
    课的教师读得到这课产出的每一个项目，正如出题者读得到自己这道题产出的项目。
    """
