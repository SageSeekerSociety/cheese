"""课程里的人：这门课有哪些成员、他们各自的项目、谁和谁一组。

课程模板的「成员与分组」（管理员）那一屏的数据。三样东西全部来自别处已有的
记录，这一格只是把它们按**一门课**摆到一起，不新造任何一份数据：

- ``space_member`` 说谁在这门课里（#1295：加入即成员）；
- 项目（锚在这版某道赛题上）说每人在做什么 —— 一个成员一学期一个项目；
...
只对本版管理员开门，判据是 ``app.auth.space_access.is_space_admin`` ——
...
class CourseRosterService:
    ...
    async def my_group(self, space_id: int, handle: str | None) -> dict[str, Any]:
        """成员自己视角的那一行：我在这门课里的项目、我挂在哪个组上。

        成员看不到花名册（那要管理员），但他至少要知道自己在哪个组、组里还有谁 ——
    ...
    async def roster(self, space_id: int) -> dict[str, Any]:
        ...
        三张表分开返回而不是在这里拼成「成员 → 项目」，是因为拼它要人的 handle
