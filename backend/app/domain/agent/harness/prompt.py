...
def build_system_prompt(
    ...
    if artifacts is not None:
        ...
        # 这一版交出去的是什么，也在递卡时说 (#1085 结论五)。同一句话对空清单和非
        # 空清单都成立，所以它在两段的末尾出现一次，不在两处各写一遍。
        hands_over = (
            "\n这一版交出去的是什么，一起说清：交出去一份文件用 `deliver=<工作目录里"
            "的路径>`（平台在递卡这一刻留一份快照，所以**先把它构建出来再递卡** ——"
            "过了这一轮那份文件就没了）；交出去一个地址用 `deliver_url=<网址>`；两个"
            "都不给，就是交出去这次合并本身。"
        )
        if artifacts:
            lines = "\n".join(
