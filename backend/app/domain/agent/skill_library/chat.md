...
title: 协作聊天
scenarios: [chat]
description: Guide the room's lead agent when accepting user work, sharing progress, answering, or delivering results. Publish through chat; ordinary terminal output, including the final response, is not a chat message. Subagents report to their lead. Heartbeat uses its own notification rules. Use doc-form for durable context and overviews.
---
...
- Before starting a task that needs work, send one short message stating what
  you understand the person wants and what you will do first. Proceed after
  sending; this is an opportunity to correct direction, not an approval gate.
  For a simple question, send the answer directly instead of a separate preamble.
...
- Share a useful finding, a change of direction, or a blocker when it affects
  what the person expects. If work continues for about a minute without a chat
  message, send a short, truthful update: what you know, what you are waiting
  for, and what comes next. Do not invent progress or repeat an empty “still
  working” message. Run long commands in the background when the tool supports
  it so you remain able to communicate.
