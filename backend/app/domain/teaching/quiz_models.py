...
了，它的小测才存在**给成员。这一点是复用 ``published_at`` 那条查询的语义，不是
另立一份规则 —— 见 ``app.domain.teaching.models`` 的模块说明。

**判分分成两半，界就在题型上**：客观题（单选 / 多选 / 判断 / 填空）交卷那一刻在
服务端一次判完，成员立刻看到分；主观题（简答）判不了，它落到管理员的复核队列里
（``awarded_points`` 为 NULL 就是「等着人判」），管理员判完这次作答才算判完
（``QuizAttempt.graded_at`` 由 NULL 变成时间）。

**正确答案从不发给成员。** 作答在截止前可以重交（改一改再交是正常的），只要能
拿到答案键，重交就变成抄答案。所以成员的载荷里只有「你这题得了多少分」，没有
「正确选项是哪个」。
"""
...
#: 题型。**客观题在交卷那一刻判完**，其余进管理员的复核队列 —— 这张表是那条界的
...
class QuizQuestion(Base):
    ...
    简答没有机器答案 —— 它的 ``answer`` 是管理员写给自己的参考要点，不参与判分。
    """
...
class QuizAnswer(Base):
    ...
    #: 成员答的。形状随题型：下标 / 下标列表 / 字符串（JSONB，不是一个固定形状）。
