...
class UserRepository:
    ...
    async def create_user(
        ...
        *,
        ...
        """Insert a user. ``user_id`` pins the row's id instead of letting the
        identity sequence choose it — agents are numbered from their own range
        (``app.domain.identity.uids``), and the column is ``GENERATED BY
        DEFAULT`` so an explicit id is accepted and leaves the human sequence
