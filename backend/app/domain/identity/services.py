...
from app.domain.identity.uids import AGENT_UID_SEQUENCE
...
class IdentityService:
    ...
    async def _next_agent_uid(self) -> int:
        """Draw the row's id from the agents' own sequence, not the humans'.

        The database hands these out, so two agents enrolled at the same instant
        cannot collide. Nothing reads the range back to decide whether a row is
    ...
    async def _create_agent_user(self, *, handle: str) -> User:
        ...
        The id is pinned rather than auto-assigned so the row lands in
        ``app.domain.identity.uids``'s range. ``user.id`` is ``GENERATED BY
        DEFAULT``, so the explicit value is taken and the human sequence is left
        untouched — the two ranges never consume each other's numbers.
        """
        return await self._users.create_user(
            user_id=await self._next_agent_uid(),
            username=handle,
