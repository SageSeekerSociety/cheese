...
认 handle（I11）—— 房间的名册、@ 的目标、`cheese notify --to` 给的都是 handle；而
知是那一侧的收件箱按数字 id 查。所以同一个收件人在这一行上有两个名字，两个都填。

handle 在账号池里找不到对应的行时 `receiver_id` 是空的：少一条知是那一侧读得到的
记录，好过把一条本该送到的通知整条丢掉。
...
class Notification(Base):
    ...
    #: 也带着键（`alert:<原 uuid>:<收件人>`），所以那次搬家重跑一遍不会多出行来。
