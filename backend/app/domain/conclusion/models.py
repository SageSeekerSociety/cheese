"""结论卡 (conclusion card) — the receipt a thread's conclusion gets.

`conclude` used to be "pour text into the room's living doc": it DID leave a
trace (a message, a doc section, a change-alert) but there was no receipt, no
status and no idempotency — the work stayed open forever and every re-conclude
appended another section to the room's doc.

A conclusion card fixes the receipt/status half of that. Its receiver is a
PLACE, not a person (`receiver_topic_id`):采信 a research conclusion needs
...
class ConclusionStatus(enum.StrEnum):
    open = "open"
    # 默认终态：房间那一轮结束（或 30 分钟绝对超时）时自动到这里。
    accepted = "accepted"
    # 补证据：唯一能回到 open 的分支。命名刻意避开「驳回/rejected」——
    # 卡面的动词直接影响 AI 的行为倾向，这条是设计要求，不是措辞偏好。
    returned = "returned"
    # 转人向：这个结论需要以某人的名义做出去，超出「房间信不信」的范围。
    escalated = "escalated"
    # 结算前那条支线又 conclude 了一次：老卡作废（顺带解决幂等）。
...
#: 绝对超时：房间那一轮若根本没跑起来（排队、崩溃、被掐），卡也不能永远挂着。
DIGEST_TIMEOUT_MINUTES = 30

#: 收起待办 —— 结论被采信了，但这条支线还挂着一张
...
#: 再归档一次。
ARCHIVE_DEFERRED = "这条支线还挂着未决的验收卡，收起推迟到验收卡有结果之后"

#: 待办消解后的落款。只要不再等于 ARCHIVE_DEFERRED，这张卡就退出扫描。
ARCHIVE_DEFERRED_DONE = "验收卡已决议，推迟的归档已经落下"

#: 验收卡决议后，留给那条支线重新递卡的宽限。驳回/作废的语义是"回去改了再来"，
...
class ConclusionCard(UuidPk, Timestamps, Base):
    ...
    __table_args__ = (
        # The two hot lookups: "this thread's live card" and "cards this room
    ...
    # 收方是地点不是人 —— the room (本体) that settles this card.
