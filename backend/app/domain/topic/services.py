...
def _as_utc(when: datetime | None) -> datetime | None:
    """Read an instant as UTC when it carries no offset.

    Query strings routinely arrive as `2026-08-12T00:00:00` with no zone; the
    columns they are compared against are TIMESTAMPTZ, so a naive value has to
    be given one before it reaches the driver. The same applies to a value read
    back out: some drivers hand back a naive datetime, and a naive one raises
    rather than merely reading wrong when subtracted.

    NOTE: there were briefly TWO functions by this name in this file — this
    None-tolerant one and a None-rejecting twin further down, landed by parallel
    changes. Python keeps the LAST definition, so the tolerant one was dead code
    and every optional caller (`active_since` unset, i.e. any unfiltered topic
    list) died on `'NoneType' object has no attribute 'tzinfo'`. Hence the
    overloads: one function, and both call shapes stay exactly typed.
    """
