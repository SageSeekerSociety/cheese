...
`.doc` / `.ppt` / `.xls` 不是 zip，是另一套二进制格式，上面那些办法一个都不适用。

- **读**：`.doc` 和 `.ppt` 先用 pandoc 试一下，能不能读取决于文件本身；`.xls` 读不了
  （见 `sheets.md`）。读不出来就如实说读不出来。
- **改**：不要改。要修改就请用户在 Office 里另存为 `.docx` / `.pptx` / `.xlsx` 再发一次，
  **并且明确告诉他这是一次格式升级**——另存出来的文件和原件不是同一个东西，后面要发给
  别人的是哪一份，得由他决定。

这条不是嫌麻烦：改这类文件要重写二进制结构，做成什么样自己验不出来；而升级格式这件事
用户看得见，出问题也知道出在哪一步。
