...
两个人同时改一份 `.docx`，合并时会撞车。**不要把两份文档的内容合起来** —— 办不到，
而且 git 不会在二进制文件里留冲突标记：它把你这一版留在工作目录里、只报一句撞了。这时
候直接提交，看起来像解决了冲突，实际上是把对方那次修改整个丢掉，而且哪里都不会报错。
...
git show "$(git merge-base HEAD <目标分支>):报告.docx" > 共同起点.docx
...
第 3 步是这条路上唯一的保险。重做可能把一处小改动放大，也可能落在错的位置，这两种都不
会自己报错；这一步过了才说得出「两边的改动都还在修订里，别的地方一个字没动」。

**`--base` 给的是共同起点，不是他那一版。** `validate --base` 的做法是拒绝掉文件里全部
修订再逐字对比，而重做出来的那一份里躺着两个人的修订 —— 拒绝全部之后它回到的是两边共同
的起点。拿他那一版当 base 会报一个假问题：他那一版自己还带着没被接受的修订。

两边的改动里只要有一处不是修订（有人用过 `--plain`），这个检查就不成立。那时候用
`text` 把两版正文摆出来自己核对，并在递卡时说明这一份没能通过 `validate`。

**重做落不下去就停下来，交给人。** `edit` 找不到原文会报错、不会猜 —— 那说明这句话在他
那一版里已经不是原来的样子（多半他也在改这一句）。这时候不要改写替换条件去凑，用
