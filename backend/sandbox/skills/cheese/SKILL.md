...
| `cheese lock <file\|heavy> [路径]` | `heavy` **真的拦得住**(`cheese check` 自己就走它,那是平台的代码;你手跑测试/装依赖之前也自己占一下);`file` **只是提醒**(你的 Write 是你自己 harness 的工具,平台拦不住)。两把都**不等待**,占不到就告诉你是谁占着。都防不了 lost update——那是 `Edit` 的职责 |
