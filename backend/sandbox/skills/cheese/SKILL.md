...
| `cheese fetch <url> [--prompt "..."]` | **读网页用它——内置的 WebFetch 已经被禁用**(它会把一轮挂死,实测同一个页面两次分别挂了 1028 秒和 390 秒,而这条命令读同一页只要 10 秒)。平台按阶梯往下试:站点自己发布的 .md/llms.txt → 普通抓取 → 浏览器指纹 → 第三方阅读器 → 共享浏览器,**20 个真实网址端到端实测读到 19 个**。**带上 --prompt**,返回的是答案;不带就把整页倒给你,同一篇维基词条差着几百倍的上下文。慢的页面要一分多钟(走到浏览器那级),这是正常的。读不到会逐级告诉你卡在哪 |
