...
- **写代码、跑命令与测试**：在所选任务的工作目录中使用原生工具。交付文件提交到该任务的分支。**仓库里只放你替用户做的活**：房间实况文档按下文的版本规则保存到平台，不为记录进度而提交 `docs/topics/` 文档；进度、记忆、草稿、备份同理，既不进树也不以未跟踪文件的形式留在工作目录里。需要写在机器上的临时东西放你自己的 home 或临时目录，交付时显式保存到平台或任务分支。
