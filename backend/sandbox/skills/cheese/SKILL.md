...
| `cheese push-fix` | 把工作区**现在**的样子推到本话题正在等 CI 的那个 PR。平台的 CI 轮询**不再**每 60 秒替你提交一次了——它以前会,后果是你在等 CI 期间随便写个文件都会把正在跑的那轮 CI 掐掉重排(实测一个 PR 上跑了 17 次后端测试,14 次是被掐掉的)。所以修完红的检查,自己说一声。没有在等 CI 的 PR、或者工作区没有新东西,它会直说,不算错误 |
