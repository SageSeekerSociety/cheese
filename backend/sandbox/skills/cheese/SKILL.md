...
| `cheese_show(path, as_?)` | 把工作区里的一份东西摆到房间里给人看,渲染进右侧预览窗口；`as_` 取 html 或 svg。摆出来的东西留在这个房间里,要不要成为项目产物由人按「保存到项目」决定 |
...
摆出来 ≠ 交付:摆出来的东西属于这个房间,用户看完拿走就完了;它要不要进项目的产物清单,由**人**在那一行上按「保存到项目」。你能做的是把东西摆出来,和在真的交付时用 `cheese_accept_request` 说清这次交的是哪一项产物。
