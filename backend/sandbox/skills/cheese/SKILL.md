...
**不是每个话题工作区都是 jj。** 跑在托管机器上的话题拿到的是一份普通 `git clone`(判据:`ls -d .git` 有东西)。那边上面这一节几乎条条相反,而且错了要付代价:

- **改动不会自动进版本库,你得自己 `git commit`。没提交的东西不会进 PR。**
...
| `cheese push-fix` | 把工作区**现在**的样子推到本话题正在等 CI 的那个 PR。平台的 CI 轮询**不再**每 60 秒替你提交一次了——它以前会,后果是你在等 CI 期间随便写个文件都会把正在跑的那轮 CI 掐掉重排(实测一个 PR 上跑了 17 次后端测试,14 次是被掐掉的)。所以修完红的检查,自己说一声。没有在等 CI 的 PR、或者工作区没有新东西,它会直说,不算错误。但它也会告诉你**工作区没能提交**——那一条要当真:改动还在磁盘上,还没进任何分支,PR 的 head 一动没动,再等 CI 是白等 |
