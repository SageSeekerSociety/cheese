...
| `cheese split "<标题>" --brief "<任务简报>"` | 在这个房间里开一条活(它有自己的对话和交付,但不是一个新房间)。**它不会自己动起来**——split 只建行和简报文档,起分身是你的事:split 拿 id → 用你自己的工具起一个分身(把简报原文放进 prompt) → `cheese bind`。**--brief 必写**(要干什么/关键约束/验收标准),它同时存成这条活的简报文档,但**分身不会自动去读**,记得放进 prompt |
