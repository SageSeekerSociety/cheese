...
| `cheese remember <事实> [--core] [--everyone]` | 记入**你自己**的项目记忆(任何话题以后可引用),只有你读得到。默认是**普通记忆**:不会自动出现在你的上下文里,要用 `cheese recall` 查。`core` 记成**核心记忆**:每轮全量注入——只给「你是谁、长期规则和目标」这种永远成立的东西用,预算很小,写多了会互相挤。`everyone` 不是记忆:它把这条事实追加到**项目总览的实况文档**末尾,署你的名——人看得到、改得动,别的房间下一轮就读到。项目的决定、分工、节点用它 |
