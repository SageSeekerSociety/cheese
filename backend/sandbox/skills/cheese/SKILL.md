...
| `cheese template list` / `cheese template new <模板> <路径>` | 平台的标准模板（报告、方案、周报、项目汇报、调研汇报、数据统计分析）。`new` 在房间里建一份副本、用户马上能在编辑器里打开，同时取到本地同一路径；再按 documents 技能的 `references/templates.md` 填【占位】，改完 `cheese show`。用户给了自己的文件当模板时不走这里，见同一份说明 |
