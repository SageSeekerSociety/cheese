...
| `cheese conclude-task <task_id> "<结论>"` | 分身报完成、**你验过货之后**,替它把结论落成结论卡。不自动:一个分身可以报好几次完成(把长命令丢进自己的后台再停下等也算一次),完成信号不等于活干完了 |
...
| `cheese accept-request <handle> "<理由>" --subject "<提交标题>" [--body "<为什么>"]` | 成果做完,把验收卡递给某个具体的人。**只有房间递得了卡,支线递不了**(后端会拒):一棵树=一个分支=一个 PR=**一批**活,而这批活是整个房间的——递卡把分支封口开 PR,兄弟支线还在往同一条分支上写。支线做完用 `cheese conclude` 把结论交回房间,由房间统一递卡。**`--subject` 必填**:平台是 squash 合并,整个话题最后塌成**一个 commit**,而它的标题就是这里写的这句(同时也是 PR 标题)。格式走 Conventional Commits(`fix(scope): 描述`),后端会当场校验(类型前缀、≤72 字、结尾无句号、不能是中文),不合格直接打回——所以别写话题名,那在 `git log` 里等于没写。`--body` 只在标题说不清时写,讲**为什么**(根因、为何是这个改法),不要罗列改了哪些文件 |


> 配了质量闸门（check_command）的项目：递卡后平台会自动跑检查，红了卡片会打回并叫你去修——**递卡前先自己把检查跑绿**，省一个来回。
| `cheese conclude "<结论>"` | 这条支线做完,把结论回流到它所在的房间 |
| `cheese tell <支线\|parent> "<一句话>"` | **上下之间随时传话,双向**:房间 → 它派出去的支线、支线 → `parent`(它所在的房间)。收方**当场被叫到**——它正在跑一轮就直接插进那一轮(秒级),闲着就叫醒它,同期来的多条合成一轮。用在:简报写错了/口径变了/发现一个前提不成立——**别等对方交完活再返工**。支线问一句也用它(要按钮拍板才用 `cheese ask`)。不在同一条上下关系里的两个地方发不通。收方写话题 id、`<#id>` 或标题都行 |
| `cheese conclusion <accept\|need-evidence\|escalate> <卡id> ["<理由>"]` | 结算支线回流上来的结论卡。**默认采信**:你这一轮结束时 open 的卡自动采信、那条支线随之收起(收起不冻结,照常能追加对话),**什么都不做就是对的**。只有缺一条关键证据(`need-evidence`,每张卡限一次,会花掉那条支线一整轮)或这事得以某人名义做出去(`escalate`)才动它 |
...
| `cheese check [--timeout 90]` | 跑项目配的那条快检,在**你自己的沙箱里**、走本房间的重活车道,**不占 CI runner**,本轮立刻返回(跑完平台叫醒你)。**不是闸门**:红了照样能递卡、照样能采纳——决定权在 PR 上的真 CI(#296);它的作用是让红灯**被看见**(结果显示在验收卡上)。**超时算失败**:快检的全部价值就是快 |
| `cheese claim <路径...>` | 给自己这条活**追加**路径声明,并当场看到还有谁在同一块地方干活。同一个**文件**被两条活声明会被拒绝;**目录**重叠只警告。简报里写路径没用——简报写完改不了,而声明必然要长 |
| `cheese lock <file\|heavy> [路径]` | `heavy` **真的拦得住**(跑测试/装依赖/起 dev server 都走 `cheese await`,那是平台的代码);`file` **只是提醒**(你的 Write 是你自己 harness 的工具,平台拦不住)。两把都**不等待**,占不到就告诉你是谁占着。都防不了 lost update——那是 `Edit` 的职责 |
