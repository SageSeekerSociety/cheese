...
| `cheese conclusion <accept\|need-evidence\|escalate> <卡id> ["<理由>"]` | 结算一条活回流上来的结论卡。**默认采信**:你这一轮结束时 open 的卡自动采信、那条活随之收起(收起不冻结,照常能追加对话),**什么都不做就是对的**。只有缺一条关键证据(`need-evidence`,每张卡限一次,会花掉房间一整轮去转达)或这事得以某人名义做出去(`escalate`)才动它 |
