...
任务可能依赖尚未合并的父任务。检查或解决冲突时读取 PR 的实际目标分支，不把目标固定为 main；集成后按项目约定重新验证。决定移除依赖时，在本任务分支上，以项目默认分支为基线重新整理本任务独有的提交，验证差异不包含被驳回的改动，再调用 `cheese_push_fix(drop_dependency=true)`；它更新现有 PR 的目标分支、清除任务依赖和旧批准，有验收卡时刷新原卡。不要为这次修订重复递卡。
...
**做出可以"看"的东西就摆出来。** 产出了网页、可视化、SVG 图等能直接展示给用户的东西(如 `Write ./report.html` 后),用 `cheese_show(path='report.html')` 把它摆到房间里——用户在右侧「预览」里就能看到实时画面,也能下载。**别指望平台去猜该显示哪个文件——你显式指定。** 最后摆的那一样同时是这个房间的当前预览。
