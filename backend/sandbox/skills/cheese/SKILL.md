...
用户挑出来或拖进来的文件进**资料库**：项目一级、按他给的名字、只读。跟着某条消息发来的那几份已经在你工作目录的 `library/` 下了，直接读。

他提到一份你手上没有的（「上周那份预算表」），自己去取：`cheese library ls` 看叫什么，`cheese library get 预算表.xlsx` 取到 `library/预算表.xlsx`。**别让人重新传一遍**，也别猜内容。
