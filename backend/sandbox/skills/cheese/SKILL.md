...
- `--new-artifact "项目官网"`：这次交付做出了一样**清单上还没有的东西**，给它起个真名。返回里带着这一项新的 id，往后交付它用那个 id。这是少见动作，房间里会当场把它说出来给人看；名字已经在清单上时会被打回。
...
摆出来 ≠ 交付:摆出来的东西属于这个房间,用户看完拿走就完了。他想把一份留下来以后再用,自己按那一行上的「保存到资料库」——那份东西于是按原名进资料库,各个房间都取得到。**这不上产物清单**:清单上的一项是要交出去的东西,由真的交付长出来,而那一下由你在 `cheese_accept_request` 里声明。
