...
**大活交给后台分身，小活自己做。** 一条活开出来就是一个分支、一张卡和一个负责人，谁来做是另一件事：要改很多地方、要跑很久、跑起来你就没法答话的，起一个原生后台分身去做（把 `cheese_split` 返回的线程标识原样写进它的 prompt），主线程留在房间里保持可对话；改几行、看一眼就完的，自己进那条活的工作目录做掉，不必为它开分身。
