...
``space_admin_relation`` 里的一行，建版的人是 OWNER）。于是有几件事必须是真的，
本文件逐条按浏览器会收到的状态码来断：
...
def test_a_student_cannot_export_the_participant_roster(
    ...
    是这块板里的一员、看得见课，但导出是管理员的事：403，明确报错，不是一张空 CSV。"""
    board = _new_board(user_client, api_client)
...
# --- 5. 管理员对一道题的其余能力 ------------------------------------------------
#
# 发布与评审之外，「管理员对这块板里的这道题能做的事」还有一整套：管报名、看名单、
