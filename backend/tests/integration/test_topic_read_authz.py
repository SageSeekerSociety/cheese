...
def _project_with_a_secret(client) -> tuple[str, str]:
    ...
    p = client.post(
        "/api/projects", json={"name": "薪资", "owner_handle": "alice"}
    ).json()["data"]
    tid = p["root_topic_id"]
    r = client.post(
...
def test_an_outsider_cannot_read_the_conversation(client):
    ...
    r = client.get(f"/api/topics/{tid}/blocks", headers=session_auth_headers("mallory"))
...
def test_an_outsider_cannot_read_the_topic_header(client):
    ...
    _, tid = _project_with_a_secret(client)
    r = client.get(f"/api/topics/{tid}", headers=session_auth_headers("mallory"))
    assert r.status_code == 403, r.text
...
def test_a_member_still_reads_both(client):
    ...
    _, tid = _project_with_a_secret(client)
    head = client.get(f"/api/topics/{tid}", headers=session_auth_headers("alice"))
    assert head.status_code == 200, head.text

    body = client.get(
        f"/api/topics/{tid}/blocks", headers=session_auth_headers("alice")
    )
...
def test_an_anonymous_caller_is_still_let_through(client):
    ...
    """
    _, tid = _project_with_a_secret(client)
    assert client.get(f"/api/topics/{tid}").status_code == 200
    assert client.get(f"/api/topics/{tid}/blocks").status_code == 200
