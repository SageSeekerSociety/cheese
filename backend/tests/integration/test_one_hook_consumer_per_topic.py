...
async def test_each_channel_recovers_only_the_machines_it_owns(business_db_factory):
...
async def test_a_reconnect_does_not_hand_the_same_topic_to_both_channels(
    business_db_factory,
):
...
async def test_a_recovered_message_lands_once_and_whole(business_db_factory, tmp_path):
    ...
    router = HookRouter()
    device_runtime = ClaudeCodeRuntime(device, router=router)
    cloud_runtime = ClaudeCodeRuntime(cloud, router=router)
    pool = ComputePool([device_runtime, cloud_runtime], device.name)
    service = ChatService(
        session_factory=factory,
        compute=pool,
    ...
    consumer_tasks = [
        subscription.consumer_task
        for runtime in (device_runtime, cloud_runtime)
        for subscription in runtime._subscriptions.values()
        if subscription.consumer_task is not None
    ]
    ...
    for runtime in (device_runtime, cloud_runtime):
        for subscribed_topic in list(runtime._subscriptions):
            await runtime._close_topic(subscribed_topic)
    await asyncio.gather(*consumer_tasks, return_exceptions=True)


async def test_screens_are_adopted_with_no_transaction_open(business_db_factory):
