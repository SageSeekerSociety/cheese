...
# DIAGNOSTIC for the #721 CI hang — drop before merge. Every pytest process
# (the controller and each xdist worker) prints all of its thread stacks to
# stderr 11 minutes after this import. A green run finishes in ~6 and the job
# ceiling is 20, so this fires only on the hang, and names where each process
# is stuck — including a stall after the last test, which no per-test timeout
# can see.
