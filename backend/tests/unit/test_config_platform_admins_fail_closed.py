"""PLATFORM_ADMIN_HANDLES must fail closed when a deployment names nobody.

``/admin/*`` is opened by exactly one thing: the ``platform_admin_handles``
settings list (env name ``PLATFORM_ADMIN_HANDLES``; it was
``FEEDBACK_ADMIN_HANDLES`` and both spellings still load — see the two
...
def test_compose_deployment_rejects_it_even_when_environment_says_development() -> None:
    ...
    assert "PLATFORM_ADMIN_HANDLES" in str(excinfo.value)
...
def test_local_and_test_keep_the_empty_default(environment: str) -> None:
    ...
    assert settings.platform_admin_handles == []
...
def test_a_blank_entry_is_rejected_everywhere(blank: str) -> None:
    ...
    with pytest.raises(RuntimeError) as excinfo:
        _build(
            ...
            platform_admin_handles=[blank, "alice"],
        )
    assert "PLATFORM_ADMIN_HANDLES" in str(excinfo.value)
...
def test_prod_template_advertises_the_requirement() -> None:
    ...
    assert "PLATFORM_ADMIN_HANDLES" in keys


def test_the_old_env_name_still_loads(monkeypatch: pytest.MonkeyPatch) -> None:
    # The rename cannot edit a box's env file, and a deploy that fails to boot
    # because of a name change is a worse outcome than the wrong name. So the
    # old spelling keeps working: it is read as an alias of the field.
    monkeypatch.setenv("FEEDBACK_ADMIN_HANDLES", '["alice"]')
    assert _build(
        environment="test", jwt_secret="dev-secret"
    ).platform_admin_handles == ["alice"]


def test_the_new_env_name_wins_when_both_are_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Order matters, and it is the reason the new name is first in the
    # `AliasChoices`: a box that has both (someone edited the file by hand and
    # the deploy workflow writes the new one) must read the new one, or the
    # hand-edit silently outranks the deploy and the roster drifts.
    monkeypatch.setenv("FEEDBACK_ADMIN_HANDLES", '["old"]')
    monkeypatch.setenv("PLATFORM_ADMIN_HANDLES", '["new"]')
    assert _build(
        environment="test", jwt_secret="dev-secret"
    ).platform_admin_handles == ["new"]
