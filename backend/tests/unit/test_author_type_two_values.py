...
名字。`platform` 是新行写下的那个，`system` 只剩存量行还带着，还剩的那次
``UPDATE`` 改写完它就从枚举里离场（为什么改名要分三次发布，见
``AuthorType.platform``）。合并档位的难处从来不在改枚举，在于全仓 30 多处读点各自
用 ``author_type == ai`` 回答了一个它答不了的问题（「这句是不是芝士说的」）；只要
枚举里还留着 ``human``/``ai``，下一个人写下 ``AuthorType.ai`` 时什么也不会响。现在
它会响：那两个名字不存在了，一取就是 ``AttributeError``。
...
def test_the_column_answers_participant_or_platform_and_nothing_else() -> None:
    ...
    `system` 和 `platform` 不是两档，是同一档的两个名字：新行写下的是 `platform`，
    `system` 只剩存量行还带着。还剩一次发布，一条 `UPDATE` 把那些行改写成
    `platform`，`system` 从枚举里离场，也从这里消失。
    """
