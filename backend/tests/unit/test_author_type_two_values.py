...
第一条判据是枚举自己：成员集合恰好是 ``participant`` 和 ``platform``。合并档位的
难处从来不在改枚举，在于全仓 30 多处读点各自用 ``author_type == ai`` 回答了一个它
答不了的问题（「这句是不是芝士说的」）；只要枚举里还留着退役的名字，下一个人写下
``AuthorType.ai`` 时什么也不会响。现在它会响：那些名字不存在了，一取就是
``AttributeError``。
...
这一条有两处是踩着一个模拟脚本（已删）的真实写法长出来的，两处都是它当初漏过去的
