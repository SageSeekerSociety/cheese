...
class TestOAuthLoginRedirect:
    ...
    async def test_redirects_to_authorization_url(self):
        oauth = MagicMock()
        oauth.generate_authorization_url.return_value = "https://v.ruc.edu.cn/oauth2/authorize?x=1"
...
def _auth_service(*, existing_user=None, created_user=None):
    ...
    auth.register_from_oauth = AsyncMock(
        return_value=(created_user, SimpleNamespace(id=1)) if created_user else (None, None)
    )
    resolved = existing_user or created_user
    auth.get_user_with_profile = AsyncMock(return_value=(resolved, SimpleNamespace(id=1)))
    return auth
...
class TestOAuthCallback:
    ...
    async def test_existing_binding_logs_in(self):
        info = OAuthUserInfo(id="uid-1", email="a@ruc.edu.cn", name="A", preferred_username="2021")
        ...
        resp = await handle_oauth_callback(
            "ruc", code="c", state="s", session=session, oauth_service=oauth, auth_service=auth
        )

        loc = _location(resp)
        assert resp.status_code == 302
        assert loc.startswith(f"{settings.frontend_url}{settings.frontend_oauth_success_path}")
    ...
    async def test_email_match_links_existing_account(self):
        info = OAuthUserInfo(id="uid-2", email="b@ruc.edu.cn", name="B", preferred_username="2022")
        oauth = _oauth_service(user_info=info, existing_connection=None)
        auth = _auth_service(existing_user=_fake_user(9, "b@ruc.edu.cn"))
        session = MagicMock(rollback=AsyncMock())

        resp = await handle_oauth_callback(
            "ruc", code="c", state=None, session=session, oauth_service=oauth, auth_service=auth
        )
    ...
    async def test_new_user_is_auto_provisioned(self):
        info = OAuthUserInfo(id="uid-3", email="c@ruc.edu.cn", name="C", preferred_username="2023")
        oauth = _oauth_service(user_info=info, existing_connection=None)
        auth = _auth_service(existing_user=None, created_user=_fake_user(11, "c@ruc.edu.cn"))
        session = MagicMock(rollback=AsyncMock())
    ...
    async def test_new_user_without_email_gets_synthetic_email(self):
        info = OAuthUserInfo(id="uid-4", email=None, name="D", preferred_username="2024")
        oauth = _oauth_service(user_info=info, existing_connection=None)
        auth = _auth_service(existing_user=None, created_user=_fake_user(12, "ruc-uid-4@oauth.ruc.local"))
        session = MagicMock(rollback=AsyncMock())
        ...
        assert auth.register_from_oauth.await_args.kwargs["email"] == "ruc-uid-4@oauth.ruc.local"

    @pytest.mark.anyio
    async def test_exchange_failure_redirects_to_error(self):
        oauth = MagicMock()
        oauth.handle_callback = AsyncMock(side_effect=RuntimeError("token exchange failed"))
        auth = _auth_service()
        session = MagicMock(rollback=AsyncMock())

        resp = await handle_oauth_callback(
            "ruc", code="bad", state=None, session=session, oauth_service=oauth, auth_service=auth
        )

        loc = _location(resp)
        assert resp.status_code == 302
        assert loc.startswith(f"{settings.frontend_url}{settings.frontend_oauth_error_path}")
        assert "message=oauth_failed" in loc

    @pytest.mark.anyio
    async def test_account_error_rolls_back_and_redirects_to_error(self):
        info = OAuthUserInfo(id="uid-5", email="e@ruc.edu.cn", name="E", preferred_username="2025")
        oauth = _oauth_service(user_info=info, existing_connection=None)
        auth = _auth_service(existing_user=None, created_user=_fake_user(13, "e@ruc.edu.cn"))
