...
`system` is deliberately left alone. It is the platform's own events under the
name they were written with, and this release renames that member to `platform`
in the enum only. `deploy/deploy-docker.sh` runs `alembic upgrade head` before it
swaps the containers, so during the window it is the previous image that reads
whatever this migration wrote — and that image has no `platform` member. Rewriting
those rows here would hand it a `LookupError` for nearly every room's timeline,
with a rollback that swaps the image back without running a downgrade. The
rewrite belongs to the release after this one, together with dropping the
`system` member: by then the image serving the window is this one, which knows
both names.
