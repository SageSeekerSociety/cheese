...
if ! curl -sf http://localhost:5200/ >/dev/null 2>&1; then
  # Through vite's same-origin /api proxy, pointed at the demo backend: a
  # cross-origin API URL cannot refresh a sign-in.
  VITE_API_BASE_URL=/api BACKEND_URL=http://127.0.0.1:8799 \
