...
assert_plan() {
  ...
  local actual
  actual="$(printf '%s\n' "$output" | grep -E '^(backend|sandbox|frontend)=' | paste -sd, -)"
...
assert_plan 'backend=true,sandbox=true,frontend=true' ''

commit_path frontend/src/main.ts
assert_plan 'backend=false,sandbox=false,frontend=true' "$base_sha"
...
assert_plan 'backend=false,sandbox=false,frontend=false' "$base_sha"
assert_plan 'backend=true,sandbox=true,frontend=true' "$base_sha" workflow_dispatch branch
assert_plan 'backend=true,sandbox=true,frontend=true' "$base_sha" push tag
...
commit_path backend/app/main.py
commit_path frontend/src/main.ts
assert_plan 'backend=true,sandbox=false,frontend=true' "$base_sha"
