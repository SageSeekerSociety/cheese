...
type Tmux interface {
	...
	// CapturePane returns the current visible contents of the session's
	// first pane (used by the auto-driver to look for prompts).
	CapturePane() (string, error)
	// SendKeys sends a sequence of tmux send-keys arguments (literal text
	// or tmux key names such as "Enter") to the session.
...
func (m *Manager) CapturePane() (string, error) {
	cmd := m.command("capture-pane", "-t", m.sessionName, "-p")
	var stdout, stderr bytes.Buffer
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return "", fmt.Errorf("tmuxmgr: capture-pane: %w: %s", err, strings.TrimSpace(stderr.String()))
	}
	return stdout.String(), nil
}

// SendKeys sends the given literal text / tmux key names to the session's
// active pane, in a single send-keys invocation.
func (m *Manager) SendKeys(keys ...string) error {
	if len(keys) == 0 {
		return nil
	}
	args := append([]string{"send-keys", "-t", m.sessionName}, keys...)
	cmd := m.command(args...)
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("tmuxmgr: send-keys: %w: %s", err, strings.TrimSpace(stderr.String()))
	}
	return nil
}
