...
def upgrade() -> None:
    op.add_column(
        "task", sa.Column("published_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "task", sa.Column("ended_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column("space", sa.Column("visible_task_limit", sa.Integer(), nullable=True))
    op.execute(
        "UPDATE task SET published_at = updated_at WHERE approved = 0 AND published_at IS NULL"  # noqa: E501
    )
