...
// A pooled process learns its room only after startup. Check the actual model
// request and hook environment, without relying on a model to repeat a marker.
func TestRendezvousBindsPrivateProjectContext(t *testing.T) {
	claude := claudeBinary(t)
	version, err := exec.Command(claude, "--version").Output()
	if err != nil || !strings.HasPrefix(string(version), pinnedVersion) {
		t.Fatalf("requires native %s: %s (%v)", pinnedVersion, version, err)
	}
	root := realTempDir(t)
	// Native background tasks can still be unwinding after the tmux server exits.
	// Registered before the manager cleanup, so only this test's files remain.
	t.Cleanup(func() {
		deadline := time.Now().Add(5 * time.Second)
		for {
			if err := os.RemoveAll(root); err == nil || time.Now().After(deadline) {
				return
			}
			time.Sleep(100 * time.Millisecond)
		}
	})
	home, work, project := filepath.Join(root, "home"), filepath.Join(root, "neutral"), filepath.Join(root, "project")
	config := filepath.Join(home, ".claude")
	for _, path := range []string{config, work, project} {
	...
	ready, receipt := filepath.Join(root, "ready"), filepath.Join(root, "receipt")
	hookSettings := map[string]any{"hooks": map[string]any{
		"SessionStart":     []any{map[string]any{"hooks": []any{map[string]any{"type": "command", "command": "printf 'started\\n' >> " + ready}}}},
		"UserPromptSubmit": []any{map[string]any{"hooks": []any{map[string]any{"type": "command", "command": "printf '%s' \"$CHEESE_CONTEXT_PROBE\" > " + receipt}}}},
	}}
	settingsJSON, _ := json.Marshal(hookSettings)
	write(filepath.Join(config, "settings.json"), string(settingsJSON))
	gates, _ := json.Marshal(map[string]any{
		"hasCompletedOnboarding": true, "bypassPermissionsModeAccepted": true,
		"projects": map[string]any{work: map[string]bool{"hasTrustDialogAccepted": true}, project: map[string]bool{"hasTrustDialogAccepted": true}},
	})
	write(filepath.Join(config, ".claude.json"), string(gates))
	const platformMarker = "PLATFORM_BOUND_CONTEXT_7319"
	const projectMarker = "PROJECT_BOUND_CONTEXT_8527"
	const prompt = "Verify the bound context: FIRST_BOUND_INPUT_9641"
	...
	api := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if json.NewDecoder(r.Body).Decode(&body) == nil &&
			bytes.Contains(body.System, []byte(platformMarker)) &&
			bytes.Contains(body.Messages, []byte(prompt)) {
			select {
			case requests <- bytes.Contains(body.Messages, []byte(projectMarker)):
			default:
			}
		}
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"type":"error","error":{"type":"invalid_request_error","message":"Isolated request captured"}}`))
	}))
	t.Cleanup(api.Close)
	isolateTmux(t)
	...
	t.Cleanup(func() { manager.KillServer() })
	sock := filepath.Join(os.Getenv("TMPDIR"), "bind.sock")
	sess, err := manager.Spawn("bind-context", []string{"sh", "-c", "cd " + work + " && exec " + claude + " --dangerously-skip-permissions"}, []string{
		"HOME=" + home, "CLAUDE_CONFIG_DIR=" + config,
		"ANTHROPIC_BASE_URL=" + api.URL, "ANTHROPIC_AUTH_TOKEN=mock",
		"CLAUDE_CODE_OAUTH_TOKEN=", "ANTHROPIC_API_KEY=",
		"DISABLE_AUTOUPDATER=1", "DISABLE_TELEMETRY=1",
		"NO_PROXY=127.0.0.1,localhost", "no_proxy=127.0.0.1,localhost",
		"CLAUDE_BG_BACKEND=daemon", "CLAUDE_BG_RENDEZVOUS_SOCK=" + sock,
		"CLAUDE_BG_RV_AUTH=binding-test",
	}, paneCols, paneRows)
	if err != nil {
		t.Fatal(err)
	}
	waitFor(t, "native SessionStart", 30*time.Second, func() bool { _, err := os.Stat(ready); return err == nil }, func() { t.Logf("session: %v", sess) })
	client, err := rendezvous.Dial(context.Background(), sock, "binding-test", rendezvous.Options{WaitForSocket: 10 * time.Second})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { client.Close() })
	styles := filepath.Join(config, "output-styles")
	if err := os.Mkdir(styles, 0o700); err != nil {
		t.Fatal(err)
	}
	write(filepath.Join(styles, "bound.md"), "---\nname: BoundContext\ndescription: Isolated binding test\nkeep-coding-instructions: true\n---\n"+platformMarker+"\n")
	write(filepath.Join(project, "CLAUDE.md"), projectMarker+"\n")
	hookSettings["outputStyle"] = "BoundContext"
	hookSettings["env"] = map[string]string{"CHEESE_CONTEXT_PROBE": "BOUND_ENVIRONMENT"}
	settingsJSON, _ = json.Marshal(hookSettings)
	write(filepath.Join(config, "settings.json.new"), string(settingsJSON))
	if err := os.Rename(filepath.Join(config, "settings.json.new"), filepath.Join(config, "settings.json")); err != nil {
		t.Fatal(err)
	}
	if err := client.Reply("/cd " + project); err != nil {
		t.Fatal(err)
	}
	if err := client.Reply(prompt); err != nil {
		t.Fatal(err)
	}
	select {
	case containsProject := <-requests:
		if !containsProject {
			t.Fatal("first bound request omitted project instructions")
		}
	case <-time.After(15 * time.Second):
		t.Fatal("no model request contained the bound platform instructions and user prompt")
	}
	value, err := os.ReadFile(receipt)
	if err != nil || string(value) != "BOUND_ENVIRONMENT" {
		t.Fatalf("prompt hook did not receive bound environment: %q (%v)", value, err)
	}
	starts, err := os.ReadFile(ready)
	if err != nil || string(starts) != "started\n" {
		t.Fatalf("binding restarted the native session: %q (%v)", starts, err)
	}
}
