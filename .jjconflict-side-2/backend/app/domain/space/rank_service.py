...
class SpaceRankService:
    ...
    async def award_rank(
        ...
        if space is None or not space.enable_rank:
            return False
        await self._rank_repo.increment_rank(
            space_id=space_id, user_id=user_id, delta=delta
        )
        return True
    ...
    async def award_rank_if_higher(
        ...
        *,
        ...
        if space is None or not space.enable_rank:
            return False
        current_rank = await self._rank_repo.get_rank(
            space_id=space_id, user_id=user_id
        )
        if current_rank >= task_rank:
            return False
        await self._rank_repo.set_rank(
            space_id=space_id, user_id=user_id, rank=task_rank
        )
        return True
