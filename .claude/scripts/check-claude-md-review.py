...
them; counting that would make this gate decorative on day one. The PR's own
author does not count either, for the obvious reason.
...
def judge(pr: int) -> int:
    ...
    print(f"FAIL: #{pr} changes {GUARDED} and no human has approved it.")
    print("")
    ...
    print("")
    print(f"  -> ask someone to review and approve #{pr}. A bot approval (CodeRabbit)")
    print("     and your own do not count.")
...
def self_test() -> int:
    ...
    print("PASS: check-claude-md-review self-test (scope, bots, self, withdrawal)")
    return 0
