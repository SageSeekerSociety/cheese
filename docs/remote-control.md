...
cannot approve their own tools or change their permission mode. Use a room ID
for `{place_id}`; task card IDs are not accepted there.
...
Update the proxy addon before rolling out the backend and frontend. The current
launch credentials do not request RC, so the addon preserves their existing
