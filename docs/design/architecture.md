...
**当前每个 agent 都拥有整个项目的全部权限(能力上一律平等)。** agent 之间的"权力/层级"**不由权限实现**,而是由**角色提示词**(§6 `agent/roles`、`skills`)告知各自的责任与范围——软约定,非强制。三层各司其职:

- **权限(硬,§4)= 项目边界**:项目的 agent *集体*能碰什么 = 分享进项目的能力之并集;对项目内所有 agent 一致。
- **权力(软,§6)= 项目内的责任分工**:管理树、谁负责什么,靠提示词,不强制。
- **关注(结构,§5)**:谁听谁,编码权力的流向,并保证唤醒不成环。

因此 §4.3 的"工具白名单"是一条**当前置空(= 全部)的休眠缝**:机制留着以备将来收紧,目前不用它编码层级。**"人人满权"之所以安全,靠块 append-only + 状态派生**(§3):两个等权 agent 同改一处不是覆盖而是追加 + 引用,顶层 agent 事后可 reconcile。将来若需硬隔离,再启用白名单缝。
...
- **群聊有类型**:`general`(普通协作)与 `management`(管理群,规划中)。管理群编码**管理树**:一个**上级** agent + 若干**下级** agent。群内角色(`superior`/`subordinate`)是 **per-(agent,群)** 的(同一 agent 在自己管理群是上级、在别处是普通 worker),默认关注策略由角色派生——上级 `all_messages`(含 agent 消息,始终监督),下级 `mention_only`(被 @ 才动)。管理树不建单独的"组织架构表",它**就是**管理群构成的树(最小物化 = `group.kind` + `membership.role`)。

- **防死循环的真正不变式:唤醒图必须是 DAG。** 两种成环方式都被结构性堵住:
  - **普通群**:只有**用户发言**触发关注,agent 发言不触发别的 agent → 天然无环。
  - **管理群**:**关注的不对称本身即断环**——上级听全部、下级只应 @;上级(非 @)发言不会回触下级。所以管理群可以安全地让上级监听一切。

  agent 间协作只经**显式 @**(受控唤醒),这是刻意保留的性质。代价:上级 `all_messages` 使其 context/成本随群流量涨 → 上级尤其依赖良好的摘要/压缩纪律(做完自动 compact + 记忆 = 块投影,§3)。

- **项目里的三棵树**:**事项树**(工作项,`domain/agenda`)、**文档树**(块的 `struct_parent` 投影,§3)、**管理树**(agent 组织 = 管理群的树)。三者是**同一块底座上的不同视图**,并经块 `refs` 互指(事项 ↦ 其文档 ↦ 其归属管理群),所以"对齐"是**可导航、可对账**的。顶层 agent 被提示词赋予"尽量对齐三棵树"的责任——软目标,结构上由共享底座 + refs 支撑。
...
**已建成**(每项迁移可回滚、ruff/pyright 零、测试通过;整仓 2821 tests 绿;安全关键路径经独立对抗式评审,无授权绕过):
...
- AI→平台工具链:`agent/tools`(注册表 + 注入式上下文 + 参数类型校验 + ToolInvoker + block 工具)、`api/routes/agent_tools`(可达的 `/agent/tools` RPC,已挂进 main)。
- 芝士引擎接口:`agent/interfaces`(抽象 Agent)、`agent/adapters/{recording,naive_api}`。
- 业务域:`domain/review`(验收:accept/reject)、`domain/milestone`(deadline/日历)。
...
- naive_api 的真实 ChatModel 绑定(AsyncOpenAI 工具调用);采纳=merge 的 git 落地。
- notification 扩展(agent 决策请求)、`compute/*`(PVE provisioning)、`agent/{memory,skills,roles}`。
