...
| A subagent's own Agent call starts a nested agent, reported as `task_started` and `task_notification` for that call. The nested agent's messages are not on stdout, only its parent subagent's; its transcript is `subagents/agent-<agentId>.jsonl` beside its parent's. A workflow agent's messages are not on stdout either. The workflow's `task_progress` names each agent's `agentId` in `workflow_progress`, and its transcript is `subagents/workflows/wf_*/agent-<agentId>.jsonl` | Nested and workflow agents reach their card only by following files |
| AskUserQuestion inside a subagent fails as a tool error (`not available inside subagents`) and sends no `can_use_tool`, with or without the stdio prompt tool. With `--disallowedTools AskUserQuestion` it is not offered, and a call from the main thread or a subagent is a `No such tool available` tool error; the turn continues and ends with `is_error: false` | Questions only come from the main thread, and disallowing the tool is safe |
| With `--permission-mode bypassPermissions`, no `--permission-prompt-tool` and AskUserQuestion disallowed, Bash, Write under `.claude/` and outside the workspace, WebFetch and an MCP tool all run, and no `control_request` arrives | In this configuration the driver never has to answer a permission request |
