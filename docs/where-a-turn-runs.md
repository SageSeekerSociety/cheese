...
一个话题在机器上留下两样东西：这台后端上的 git worktree（`<workspace_root>/.worktrees/<项目>/topic_<id 前 8 位>`），和跑它的那台设备上的隔离 home（`$HOME/.cheese/home/<项目>/<话题>`，装着 claude 的会话文件和缓存，一个 1-4 GB）。**归档时两样都退掉**（`topic/retire.py`）：先释放 Cloud 机器，再删这个房间写过的每一棵树的 worktree，再到它钉住的那台设备上删 home，设备不在线就先留着。分支不动：提交是记录，工作区只是一份检出，没提交的东西随归档一起放弃；取消归档会从分支重新检出一棵空树，会话从头开始。归档够不到的由一个定时扫兜底：设备离线、归档中途进程死了、话题被直接删了、以及归档还不退存储的那段时间留下的存量。定时扫（`topic_storage_sweep_interval_s`，默认一小时）把盘上和每台在线设备上的条目逐个对回数据库，对不上的、或者归档超过 `topic_home_retention_days`（默认 30 天；HOME 里的 `.claude/projects/**/*.jsonl` 是原始会话记录的唯一一份，房间里的对话在 `blocks` 表里，这份不在）的删掉，每一条删或留都写一行日志说明理由。合并用的 `_merge` 暂存树不归它管。
