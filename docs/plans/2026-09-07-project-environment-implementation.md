...
3. Pass the room configuration through ChatService into the shared DeviceChannel launcher. Initialize tools before starting the room agent and preserve live-process reattachment. Run startup from the task CLI after preparing each task checkout. Test room launch followed by task creation, reopening, and failed dependency installation.
