...
Run private chats and ordinary-room Claude Code sessions on a configured central device. Keep each ordinary room's checkout, shell, environment scripts, custom MCP processes and preview on its existing execution device. Preserve conversation history when moving an existing session.
...
4. Add execution bootstrap beside the existing launch helpers. Reuse checkout preparation, environment runner, Cheese CLI and preview helpers. Ship the execution runtime to the selected machine and refresh scoped credentials without copying model credentials there. Keep project hooks in the execution environment.
