...
| 7 回收 | 地点没有期限 | I19 | **房间那一半**。DECISIONS 24 的两张收据 + 39 的休眠态；CI runner 那一半不在 |
...
`session_placement`（L0 的表上挂着 L2 的租约，#1259 因此被一个无关迁移打挂，#1308／#1347 把它拆到 `agent_sessions` 上）。
...
族 1 的离线编解码链（6，靠 DECISIONS 23）、族 7 的房间磁盘那一半（5，靠 DECISIONS 24 的两张收据）。
...
曾经有一条 `agent-harness.spec.ts`，名字看着像在测 harness，实际测的是新建队友对话框里的
两个下拉框（模型列表是环境变量喂进去的固定值，没有任何推理供给）——那两个下拉框随
「模型绑在活上、骨架是部署设置」一起退了场，这条用例也跟着走了。
