> 状态：**研究报告，第 5 节五条已拍板**（2026-09-19）。材料是 09-05 以来合并的 200 个 PR、当周事故记录与 CI 运行史；文中引用的 `families-*.md` / `audit-*.md` 是研究过程文件，未入库。讨论去 issue。
...
五条抱怨的原话在 `COMPLAINTS.md`；架构结论在 `../arch/DECISIONS-2026-09-19.md`（44 条）。
...
**178 次族归属里，44 条架构结论能根除的是 28 个（16%），能把另外 29 个从「线上发现」降级成「矩阵少一格」，
剩下 121 个（68%）一条结论都没碰到。**
...
上一版 `ARCHITECTURE-2026-09-19.md` 第 0.3 节那张不变量表，是这些族的另一种写法。对照：
...
| 3 一个取值两件事 | fallback 掩盖失败；`None` 当默认分支入口 | I19（fallback 的可见度和它掩盖的失败一样高）、I20（`None` 不许当默认分支）、I23（掩盖物理事实的留，掩盖配置错误的删） | **一小半**。DECISIONS 23 把「机器够不着」从返回值搬进事件；其余（S3 桶不可达、死 FUSE 挂载、上游同步失败）是实现纪律，44 条里没有 |
| 4 失败没有读者 | 收件人从来没有按「谁能行动」设计过 | I10（通知只有一条规则）、I22（机器状态以事件抵达，不以工具报错抵达） | **根除一半**。DECISIONS 14/15 定了事件落点与唯一收件人规则，#1128（平台自己在等一台机器开机，却每两秒告诉所有人）和 #1105（后台任务崩了，下一步在人手上，通知数是 0）两个方向同时消失。后台异常无人读那一半（#1116）不在结论里 |
| 5 锁的作用域 | 持锁跨网络 | 无 | **不消失**。DECISIONS 40「平台不设等待超时器」说的是不等，不是不持锁 |
| 1 跨进程无契约 | 一房一机的残留 + 三个发布单元共享 schema | I3（内层的死亡不得改变外层的任何一行） | **一小半**。DECISIONS 23 消掉离线那一半的编解码链；发布拓扑、发布顺序、方法白名单，44 条完全不谈 |
| 12 发布掐断长连接 | 地点不是租约，连接断了是事故不是状态 | I13（销毁前先取回） | **一半**。DECISIONS 24/39 让「连接断了」变成租约语义里的正常状态；前端 chunk 那一半（#1099）不在 |
| 7 回收 | 地点没有期限 | I13 | **房间那一半**。DECISIONS 24 的三张收据 + 39 的休眠态；CI runner 那一半不在 |
| 11 代价线性放大 | 平台自召唤与定时全量扫描 | 无 | **一半**。DECISIONS 13/16 解散 `domain/scheduler/`、把 5 处平台自召唤改写成事件，去掉一批轮询；**设备重连时的恢复扇出不在任何一条里** |
| 2 CI / 6 前端 / 13 闸门 / 14 输出两处真相 / 15 文档 / 16 WS | — | — | **无**。44 条是产品架构，不谈 CI、不谈前端渲染契约、不谈交付流程 |
...
| 5 锁的作用域 | 13 | 结论不谈事务边界 | 一条全局不变量（下文「连接不跨 I/O 边界」）+ 每把锁一条「持有 X 期间对同一行 `SELECT … FOR UPDATE NOWAIT` 必须成功」 |
...
| 16 WS 生命周期 | 3 | 44 条不谈连接活性 | 缝 1 的 acceptance（真连接器进程）+ 一条 ASGI 消息序列断言：`accept` 之后抛 `DeviceOffline`，发出的消息序列必须是 `[websocket.accept, websocket.close]`（见 §3.3 的第五种形状） |
...
2. **每条缝恰好一个替身，而这个替身是契约的执行者，不是 mock。**
   ...
   而 `httpx.MockTransport` 那种「自己造一个邻居的响应」是最坏的一种：**替身撒谎，没有东西拦它**。
3. **缝的清单分两种来源，不要混。**
   缝 A（结论 29/30/43）、B（23/24/39/40）、E（13/14/15）、F（21）、G（42/8/9）是 DECISIONS **已经定死的接口**，
   一个接口就是一条缝；接口之外的东西不许有跨模块的约定。
   缝 C 和缝 D 不是：结论 21/22 说的是**平台 MCP 在哪一侧、项目工具在哪一侧**，
   它没有定义 `execution.call/data/result` 这条线的帧格式；「托管方契约」在 44 条里**没有任何对应条目**
   （下表那一格的判据引的 I17 来自上一版 `ARCHITECTURE-2026-09-19.md`，不是 DECISIONS）。
...
1. **「持有 X 期间 Y 仍可用」**——`FOR UPDATE NOWAIT` 对偶、占住 shell 再问工具列表。判据机械，可对每把锁机械生成（族 5、族 1）。
...
反过来，本批里明确被证明**没用**的形状，新测试里禁止出现：源码断言（断言 CSS 里写了 `overflow-y: auto`）、
「结果是数字」这类只验类型的自检（#1138）、跑在 `tmp_path` 上的路径测试（#1156）、
读「下一帧」而不是「它要的那一帧」的 socket 断言（#1197）、只比键和占位符不比渲染结果的 i18n 闸门（#1202/#1166 各用负向对照证明过）。

### 3.1 七条缝（九行，A 和 C 各拆两行），各自的替身、内容、速度、层、门槛
...
**这一列要等 §4.3 的方案 A 或 B 落地才成立**；在那之前它是「应当必过」，不是「会被拦住」。
...
| **A-2. 骨架契约：原生子 agent 三项**（结论 43） | `FakeSubagent`：可被要求「指定模型 / 发一条带线程标识的事件 / 被父线程改指令 / 被停掉」 | 三项硬性要求各一条；hook 按线程标识归卡；子 agent 与父进程同生同死 | ≤ 20s | contract | **没有**（结论 43 是新设计，178 里一个 PR 都不落在它身上） | 随该部分实现一起交付，在那之前不占 PR 预算、不进门 |
| **B. 地点契约**（结论 23/24/39/40） | `LedgerPlace`：一个记账的假地点，能被要求进入 租出 / 离线 / 休眠 / 被回收 四态，并记下每一次租、问能力、归还 | 租 → 问能力 → 归还三步各自的收据；**离线是一条事件而不是一次工具报错**（I22 的第二条断言：agent 读到的字符串里不含裸 HTTP 状态码）；那一轮的工具表里项目工具直接标不可用、**不让它们各自超时**；回收前三张收据（transcript 落库、记忆整理跑过、未提交工作已推）；休眠后在 reconnect window 内恢复同一台、工作区原样 | ≤ 30s | contract | **一半**。「离线是事件」这一半有被测对象（族 3 的对端状态那 6 个），四态租约与休眠（结论 39）今天不存在 | 离线那一半 PR 必过；租约四态随实现交付 |
...
| **D. 托管方契约**（今天的实现契约；44 条里无对应条目，判据里的 I17 来自上一版 ARCHITECTURE） | `LedgerForge`：一个记账的假 forge，能被要求答 绿 / 红 / 还在跑 / 未知 | 合并、提案、读检查结论三个动作；**平台读结论不算结论**（喂一个红结论，断言采纳被拒；喂一个「还在跑」，断言采纳等待而不是放行）；卡上带 forge 身份 | ≤ 20s | contract | **有**（`merge_state.py:323` 那个空循环、采纳路径 16/58 在检查跑完前合，见 §4.3） | PR 必过 |
| **E. 投递规则** | **无替身——它是纯函数** | 输入 `(事件, 名册, 下一步在谁手上)` → 输出 `(收件人集合, 原因)`。对每一类事件断言：下一步在平台手上时收件人为空，转到某个参与者手上那一刻恰好一次（结论 15）；事件落在它「关于」的那个东西上——卡的事落卡、房间的事落时间线、项目的事落总览（结论 14）；平台从不发起一轮（结论 13：今天 5 处平台自召唤，断言它们在这张表里都变成了「事件 + 收件人」） | 全表 ≤ 2s | pure | **没有**——「投递规则是一个纯函数」这件事今天不成立，收件人散在各处。族 4 里被结论 14/15 根除的那 7 个是这条缝将来的回归集 | 随实现交付；那 7 个 PR 的行为在纯函数出现的同一个 PR 里变成表里的 7 行 |
...
| **G. 记忆文件**（结论 42/8/9） | `MemoryDir`：一个临时目录 + 假时钟 | 一个事实一个文件、元数据带「关于谁、何时观察到」；回忆规则（一个池被读出来当且仅当它关于的那个东西在现场）；写入分类（任何提到某个人的判断只能进那个人的池）；可见性（关于某个人的只对当事人可见，不进房间、不进时间线、改了不通知）；整理 = agent 重写自己的文件，历史可翻、可删 | ≤ 15s | pure | **没有**——今天记忆在库里（`MemoryScope`），不是目录里的文件；178 里没有一个 PR 落在它身上 | 随结论 42 的实现一起交付，在那之前不占 PR 预算、不进门 |
...
- **连接不跨 I/O 边界**：一个计数型 session factory，断言在任何 provider client 的 I/O 边界上打开的 session 数为 0。
  ...
  配一条机械生成的对偶：对每一把行锁，「持有它期间对同一行 `SELECT … FOR UPDATE NOWAIT` 必须成功」。
...
- **基础设施失败与测试失败在 conclusion 上必须分开**：
  每个 job 开头打印一行资源快照（磁盘余量、tmpfs 用量、产物配额、上游可达性），失败时这行就在日志顶上；
  环境前置做成显式的 precondition step，失败就以那个 step 的名字失败，报成 neutral 不阻塞。
...
随之而去的是那 15 个以上互不相干的超时——`CONNECT_RETRY_WINDOW_S=180`、`OWNER_CONNECT_RETRY_WINDOW_S=60`、
`LISTING_DEADLINE_S=20`、pi 的 `STARTUP_WAIT_S=120`，全是「不知道机器状态，只好等」的产物；
结论 40 明写「平台不设等待超时器」，它们在设计上就没有位置。
**在此之前的过渡形状**：一份「这一轮还剩多少时间」的预算，从 H4 一路带到 H10，替掉各跳各管各的那 15 个常量。
...
**e2e 不该测什么：agent 回了什么、工具调用、机器、执行器。**
在浏览器里驱动它们既慢又无法定位，换成缝的验收：
「agent 回了一句话」→ 缝 A-1 的 contract（替身骨架，断言 transcript 增量与 hook 事件）；
「工具调用」→ 缝 C-2 的 acceptance；「机器离线」→ 缝 B 的 contract。
...
**1. 合并门用哪个方案。** [已定 2026-09-19 的部分 + 待他选的部分]
事实：仓库在组织 `SageSeekerSociety` 名下（免费计划，36 座位 / 30 成员），组织私有仓库的分支保护要 **Team**（$4/人/月，按 36 座位约 $144/月），个人 Pro 不起作用；转 public 免费但要先轮换贴过的 PAT、扫整个历史与 issue 正文，且等于开源产品。
[已定] **不禁止 agent 自行合并**（agent-principles 二、七）。门对谁都一样、而且是机械的：
① 本项目配 `required_checks`（`test`、`e2e`、`repo-guards`、`empty-pr-guard`）加 `strict: true`，采纳流程据此等/拒；
② agent 机器的房间 PATH 里 shim `gh`，只拦 `pr merge`：判定不自己写，调平台的采纳端点，让 `merge_state.py` 是全仓唯一的合并判定（第六条、I24）；不满足就拒绝并印出哪条红着；shim 不碰机主的 `gh`（第十六条）；
③ 平台已有的 CI 挂了回流（`pr_signals.py`）照旧把红送到写代码的芝士面前。
以上三条把 agent 那一路当天覆盖到 100%，不花钱。
[待他选] 人在自己笔记本上的 `gh pr merge`（428 里 370 次）只有 Team 或 public 能拦：Team 是一年约 $1,700 换一个开关；public 要先清理并改变产品公开程度。
