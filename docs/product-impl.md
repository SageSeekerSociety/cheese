...
- **分级限流**：判据是现成的 —— `NotificationRepository.over_quota`，每话题每天 ≤2 轻 / 每周 ≤1 强，数的是收件箱里的**行**（一条广播是名册上一人一行）。**写入路径今天一处都不调它**，所以没有通知因为超额被丢掉；决策/验收请求按 Batch J 本来就永不被限流丢弃。反馈提案卡照这个形状另立了自己的配额（`settings.feedback_proposals_per_topic_per_day`）。
