...
3. **和 §13 刚定的东西直接冲突。** §13 的结论是「子话题 commit 进母话题的同一个 PR」；
   若 task ≡ PR，那 N 个子话题就必须 N 个 PR。两条设计打架，只能留一条。

### 14.2 今天已经有什么（核实）

issue #314 已经在父话题时间线上做了一条「已派出《X》」的分隔线
（<&frontend/src/components/DispatchedMarker.vue> ＋ <&frontend/src/lib/splitMarkers.ts>）。
方向是对的，但它的实现注释自己把限制写得很硬：

> 标记只能**读时派生**，不能存：唯一还在的关系是子话题的 `parent_id` ＋ `created_at`。
> 派生得出的和派生不出的，界限很硬：能——「这个时刻，从这个房间派出去了《X》，去那边看」。
> 不能——「以下这几条消息 / 这一项待办不归这里了」。那需要一条从子话题指回父话题某个
> block 的边，**数据库里没有这条边**。

它今天能显示的只有：标题、子话题状态（active / archived）、一个跳转链接。
...
| 递了验收卡 | 检查状态（今天闸门只跑 lint，CI 在 PR 上，卡上要分清这两段） |
...
要从「派生行」变成「真卡片」，只差一件事：**split 的时候在父话题写一个 block**
（`kind=event`，`meta` 里带子话题 id），而不是像今天这样什么都不写
（`SplitIn` 只有 title / created_by / brief，不往父话题落任何东西）。

一条边换来三件事：

1. **位置固定**，不再因为分页窗口而错位 —— #314 注释里承认的那个问题直接消失；
2. **能挂状态和产出**，也就是 14.3 那张表；
3. **它就是 §13.4② 那条母→子通道的 UI 落点** —— 在卡上直接回一句话给那件活，
   后端走 `POST /api/topics/{me}/message-to-child`。三件事共用一个落点，不用各做各的。
...
**这就是那张卡。** 侧栏那份可以保留成「本房间所有活的清单视图」，
但「一件活是什么、做到哪了」的第一现场应该在时间线上。
