...
- <&backend/app/domain/usage/tokens.py>（新）：cache token 折算收成一处，三条供给（sdk / hooks / 订阅代理）各自的字段方言在这里登记。<&backend/app/domain/agent/service.py>、<&backend/app/domain/agent/harness/claude_code/hook_events.py>、<&backend/app/domain/usage/subscription_ingest.py> 全部改调它，sdk 那条漏掉的两个 cache 桶补上。
