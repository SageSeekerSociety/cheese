...
**四条修复全部做完，都在分支 `topic/c738f686` 上，等递卡。** 结果摘要：

| 改了什么 | 实测收益 |
|---|---|
| 页面缓存 + 组件保活 | 进过一次的页面第二次进不再转圈；切走再回来是原来那一屏 |
| hover 预加载 | 鼠标在侧栏停 0.15 秒就预取页面代码和消息 |
| 首屏瘦身 | 首屏 4.18 → **3.04 MB**（gzip 1.13 → **0.78 MB**）；SW 预下载 12.5 → **7.0 MB** |
| 后端热点 | 未读 244 → **40 毫秒**（−83%）；成员名册 35 条 SQL → **6 条**，−64% |

前端合并后闸门全绿：998 条测试、类型检查 0 错、eslint 0 错、样式与仓库规则闸门通过。后端 unit 3715 通过、ruff/pyright 干净，完整集成套件在跑。

下面是诊断过程和证据，保留原样——它解释了这些改动为什么是对的。
...
- ~~**后端中间件叠了 4 层**~~ **已推翻**：实测 4 层加起来只有 0.645 毫秒。原文： `BaseHTTPMiddleware`（`<&backend/app/main.py>`），Starlette 的这个基类每层都要把响应过一遍 memory stream。对几百 KB 的响应有多少额外延迟，在测。
...
| 活 | 内容 | 状态 |
|---|---|---|
| 页面缓存 + 组件保活 | 新增 `lib/pageCache.ts` + `composables/useCachedResource.ts`，覆盖五个页面；保活用白名单（登录/注册/OAuth 排除）。另加 `App.keepAlive.spec.ts` 三条哨兵，逐条做过变异验证 | ✅ |
| 预加载 | 新增 `lib/routePrefetch.ts`，停留 150ms 触发；复用 blockCache 的 2 条并发闸并补上「同话题一条在飞」；省流量/慢网/触屏三道闸；**专门有测试钉住「预取不改已读状态」** | ✅ |
| 首屏瘦身 | 编辑器延后安装（`WeakSet<App>` 守卫）、Prism 17 条白名单、SW 排除 monaco 与 `.woff` | ✅ |
| 后端热点修复 | 加索引、members 批查询、`_last_activity` 合并进排序查询、`gzip_proxied any;` | ✅ |

**收益汇总**

| | 改前 | 改后 |
|---|---|---|
| 首屏文件 / 原始 / gzip | 19 / 4.177 MiB / 1.133 MiB | **14 / 3.039 MiB / 0.779 MiB** |
| SW 预缓存 | 336 条 / 12576 KiB | **319 条 / 7042 KiB（−44%）** |
| `topic-unread` p50 | 244.1 ms | **40.3 ms（−83%）** |
| `members` | 35 条 SQL / 35.8 ms | **6 条 / 12.9 ms（−64%）** |
| `private-unread` p50 | 21.0 ms | **14.3 ms**（白捡，没针对它改） |
| 话题列表 p50 | 46.4 ms | **41.0 ms** |

**响应内容一个字节没变**——后端改的全是「怎么拿到同样的答案」。

**加索引的代价**（已知并接受）：写入 +0.026 ms/块（p50 +16%），磁盘净增 87 MB，索引条数持平 8 → 8。删掉 `ix_blocks_topic_id` 之前核对过两件事：另外四个接口的执行计划无退化（还白捡 `count_for_topic` 3.13→1.38 ms、`private_unread_counts` 转 Index Only），以及**级联删除**实测 1154 → 1135 ms 无退化（这是删索引最容易出事、平时又没人测的地方）。
