...
**状态：合并本身做完了、验证全绿、卡在 `pending`；但采纳被平台侧的属主问题挡住，需要宿主 root 才能解（见文末）。我这边不用重做。**
...
第一次点采纳报 `git checkout failed: Your local changes to the following files would be overwritten`，点名了 6 个文件。我最初据这几个文件名判断是"另一个话题留在共享底仓里未提交的活"——**这个判断是错的**，实际查下来是另一回事：

**共享底仓 `/de808b13-…/` 的工作区整个是空的。** 目录下只有 `.git` 和 `.jj`，一个工作区文件都没有；1633 个 tracked 文件**全部**处于已删除状态。git 报的"本地修改"就是这 1633 条删除，报错里点名的那几个文件只是其中的头几条，跟哪个话题都没关系。

**根因是属主**：底仓顶层目录是 `root:root 755`，而容器以 `node`(uid 1000) 运行——node 在该目录下建不了任何条目，所以 `checkout` 永远铺不出工作区。而 `.git` 和 `.jj` 反倒是 node 所有（775），所以 `git` 命令本身跑得动，很容易被误读成"工作区脏"。已实测确认：`touch` 该目录 `Permission denied`，容器内无可用 `sudo`。

顺带解释了本话题开工时那个疑点：底仓 HEAD 还停在 8-10 的 `a2691f2a`，这正是我的话题工作区那个过时基线的来源。

**影响面**：卡住的是本项目**所有**话题的采纳，不只这一张卡。

**修法**在宿主侧（把顶层目录 chown 回 node，或按平台原本的属主约定改回去），沙箱内无法自救。权限一恢复，重点一次采纳即可——分支和卡都已就位，不需要重做任何东西。
