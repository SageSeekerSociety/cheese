...
| 11 | 部署销账问错了问题 → 卡永久 `pr_open` | ✅ 已修并端到端验证 | #284 |
| 12 | 话题 `updated_at` 不跟着新块走 | 未接 | — |
| 13 | 分身与人共用身份 → 用户 @ 不到分身 | **等人拍板** | — |
...
`chat.py:1533` 有 `if h != author`（自己的消息不通知自己）。分身用的就是 wangchangxin 这个账号，所以他 @ 分身 = 他 @ 自己，**永远不产生通知**。目前用轮询绕过。

正解是给分身独立身份——**等人拍板**。（待核：#273「采纳 分身独立身份与 authz 收敛」已经合进 main，这条要么是那次没覆盖到的部分，要么是它落地后仍待配置的一步；开工前先确认，别重做。）
...
### 12. 话题的 `updated_at` 不跟着新块走

实测 `373e0f28`：`updated_at` 停在 08-11T09:06，末块已是 08-12T01:47，**差 16 小时**。任何按「最近活跃」排序或筛选的地方都是错的。
...
### 11. 部署销账问错了问题 → #284，端到端验证通过

**原症状**：采纳→PR→CI→自动合并→main 已反复跑通，卡住的是**话题归档**。
...
这三个提交其实**早就上线了**——部署拉的是 main 的 tip，它们全是 `45b6169a4` 的祖先。代码到了 dev，只是账没销。
...
**结案**：`#284` 已合并并部署（`fe949e17`）。留作验收样本的三张卡在部署后**自己走完了**，无任何手工干预：

| 卡 | PR | 卡住时长 | 结果 |
|---|---|---|---|
| `fa3e5e89` | #267 | ~12.5h | topic=archived / card=accepted |
| `f1874fa3` | #270 | ~11h | archived / accepted |
| `ff1c0b9d` | #274 | ~11h | archived / accepted |

三张的备注都是同一句，而**这句话本身是这次修复最值得留下的东西**：

> PR #267 已合并；**它自己那次部署没成功（这个提交自己没有成功的部署 run）**，但更晚的一次成功部署（`fe949e17`）已经包含这个提交。

**它归档了，同时没有假装部署成功过。** 旧代码要么死等（实际发生的），要么就得撒谎说部署成功了才能放行。新判据让「这段代码上线了」和「这次部署跑成功了」变成两件可以分别陈述的事——这才是能同时做到「不卡死」和「不骗人」的原因。

两条路径都已验证：`#280`/`#281` 走正常路径（合并间隔够开，各自拿到 deploy run，直接闭合）；`#267`/`#270`/`#274` 走异常路径（run 被并发组吞掉，靠祖先判据补上）。

两个已经钉住、**不要再走回头路**的判据教训：
...
**08-12 复核**（基线已从 `2e72632e` 前进到 `fe949e17`，即 #284）：

- **#276 全部落地**：`build.yml` ×4、`build-tmux.yml`、`deploy-dev.yml` 六处 `docker login` 全是 `${{ github.repository_owner }}`；仅剩的一处 `github.actor` 是解释为什么不能用它的注释。9a 闭合。
- `deploy-dev.yml` 的并发组**仍是** `group: deploy-dev`（不含 sha）——**这是预期的**：#284 改的是判据（「这个 commit 上线了没」），不是并发配置。run 还是会被吞，只是被吞不再等于卡死。
- 08-11 那次复核记的「本工作区 `main@upstream` = `2e72632e`、六处还是 `github.actor`」已过期，是基线旧而非 #275 失效。
