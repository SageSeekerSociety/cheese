...
`AcceptService.accept()` 触发的 `ws.merge_topic()` 原来直接在项目共享的主仓库目录上做 `git checkout`/`merge`——2026-08-09 出过真实事故：全项目采纳卡死约1.5小时，直到有人手动上宿主机删锁才恢复。这张卡要把这个单点故障从根上消除：改成每次合并都在隔离的临时 git worktree 里执行，用完即弃；子进程加超时+确认终止的兜底。
...
- **两处调用点补了 `asyncio.to_thread`**（`<&backend/app/domain/review/services.py>`）：`AcceptService.accept()` 里直接 merge 那条路径（`ws.merge_topic`）和"两阶段采纳"新开 PR 那条路径（`ws.push_topic_branch_for_github_pr`/`ws.pr_base_branch`）之前是同步阻塞调用，跟旧的 PR-based accept 路径（`push_topic_branch`/`sync_upstream`，已经包了 `to_thread`）不一致——这两条不包的话，就算 git 子进程本身被隔离了，只要它还没触发超时，整个事件循环仍然会被这一次阻塞调用卡住，别的话题的采纳请求也进不来。补齐后三条路径处理方式一致。
...
- 原有 `test_upstream.py`（7 个，含 `test_accept_conflict_is_a_state_not_a_lie`、`test_sync_conflict_aborts_and_names_files`）、`test_accept.py`/`test_accept_gate.py`/`test_accept_pr_publish.py`（29 个）、`test_review_acceptance_merge_failure.py` 全部原样通过，没有为了适配改动去改断言——这些测试原来断言的是外部可观察行为（合并结果、冲突文件、working tree 内容、accept 状态机），跟"合并具体在哪个目录跑"无关。
