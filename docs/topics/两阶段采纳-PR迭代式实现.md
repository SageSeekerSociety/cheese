...
把"采纳"从单次"合并+归档"改成三阶段：人点一次采纳 → 有可用已连接 GitHub token 则推分支开真实 PR、卡片转 `pr_open`、话题不归档 → 系统轮询 PR 的真实 CI（GitHub Actions）检查状态，全绿则自动调 API 合并 PR → 合并后**继续轮询 merge 触发的部署 workflow**（Deploy dev/test box），真正 completed+success 才最终归档。CI 或部署检查红了都唤醒芝士处理。没有可用 token / GitHub API 故障 → 直接走现在的 `merge_topic()` 老路径正常归档，不阻塞采纳（这是降级，不是错误）。
...
2. 降级路径（无 token/API 故障 → 走老的 `merge_topic()`）本次要留着，不用管以后何时删除。
...
- `AcceptService.accept()`（`review/services.py:250-390`）现状：投票达标后 `ws.merge_topic()` 本地 `git merge --no-ff`（`workspace/service.py:388`），然后 `stop_topic_container()` + 话题转 `archived`。全程无 GitHub PR/API 代码。
...
- 顺手把我自己的 `push_topic_branch_for_github_pr` 也改成用 #195 的 `_token_git_env`（env var 传 token 给 git credential helper）而不是把 token 拼进 push URL——更安全，避免 token 在 `ps` 里短暂可见。
...
- `review/services.py`：`AcceptService.accept()` 先尝试 `_resolve_pr_prerequisites()`（有 token 且项目连了仓库）→ `_accept_via_pr()`（推分支+开PR+卡片转`pr_open`+不归档）；任何一步失败（含解析前置条件本身失败）都在 try/except 里降级到原来的 `merge_topic()` 路径，不影响 accept 本身。新增 `advance_pr_card()`（由轮询调用的状态机推进：查CI→合并→查部署→归档，CI/部署失败都有对应的通知/唤醒逻辑，且都做了"同一个失败不重复通知"的去重）。
