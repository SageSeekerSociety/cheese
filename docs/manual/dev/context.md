...
1. 话题还没有名字时，第一条就是「先给话题起名」。
2. 「随时推送」的约定。
3. 这个 AI 队友的专家角色。
4. 课程的教学配置：本周进度、这周还不该用的知识点（只在课程项目里有）。
5. 这个场景的操作技能（`skill_library/` 里按 `scenarios:` 标签选出）。
6. **当前阶段的操作说明**：由 `stages.py` 按话题所处阶段（拆活、执行任务、闸门、等人采纳、冲突……）只选一段注入。是平台选、静态拼进去，不让模型自己决定读不读。
7. 项目里活跃的话题列表、产物清单、成员名册与点名方式。
8. 项目总览的实况文档和当前话题的实况文档，各限 6000 字（`OVERVIEW_DOC_CHAR_BUDGET`、`TOPIC_DOC_CHAR_BUDGET`），超了就压缩并提示用 `cheese_doc_get` 读全文。
9. 核心记忆；其余记忆不自动出现，只提示条数，要用 `cheese_recall` 检索。
10. 会话开场时的运行环境（机器、限制），标明是平台元信息而不是用户输入。
...
| 私聊（DM） | 场景技能换成 `private_chat.md`：查项目、记偏好、起草文档，要仓库或团队协作时建议转到工作话题 |
| 巡检 | 场景技能换成 `heartbeat.md`，按通知规则决定要不要发言 |
