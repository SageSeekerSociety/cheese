<template>
  <!--
    成员与分组（管理员）。这门课有哪些成员、他们在做什么、谁和谁一组。

    数据来自 `GET /spaces/{id}/course/roster` —— 成员表 + 项目 + 小队三张表在服务
    端合成的一行一行，前端只负责画。**这里没有一样是前端自己算的**：谁算成员
    （成员减去管理员）、谁和谁一组（项目的 teamId），答案都在服务端。

    今天这一屏是**只读**的：把成员放进组这件事，团队那侧只认团队自己的角色
    （`require_permission(TEAM_MEMBERSHIP)`），管理员不在成员的组里，所以他改不了别人
    的组。要支持「管理员指定分组」，得先在授权那一层加一条主张 —— 那是安全面的改动，
    不是这一屏顺手能带的。屏里那句说明把现状讲给管理员听，不摆一个按不动的按钮。
  -->
