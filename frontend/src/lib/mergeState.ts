...
/** 状态行上的词 + 圈，或 null（没有信号时不画一行「未知」吓人）。
 *
...
export function mergeBadgeOf(ms: MergeStateInfo): MergeBadge | null {
  switch (ms.state) {
    case 'clean':
      // 绿勾 + 采纳亮；圈是「等你」—— 下一步真的在人手上。
    ...
    case 'unknown':
      // 平台 lane (who=human 且没有信号) 的常态：采纳本来就纯是人的判断，
      // 不画状态行。绑了 GitHub 的卡 unknown 是「还没看过，下一拍收敛」。
      return ms.who === 'human' ? null : { label: '状态更新中', column: 'delivering' }
    default:
      break
  }
