...
function isPlatformBlock(b: Pick<Block, 'author_type'>): boolean {
  // 判「不是参与者」而不是列举平台那一档的名字：那一档正在改名，两个名字现在同时在
  // 库里，新行写的是 `platform`，`system` 只剩存量行还带着；而这份 bundle 会在已经
  // 打开的标签页里一直跑下去，跨过剩下那次发布。列名字的话，改名当天
  // 「<@bobby> 合并了 #12」会掉进人说的话那一支，渲染成完整气泡加头像，一条谁都没
  // 说过的话。
