...
// 下好几个人和好几个队友，一个三档枚举说不出是哪一个。所以这一列只剩两档：参与者
// 和平台自己，「这句是不是芝士说的」改问署名（`author`，一条 handle）。
//
...
function isPlatformBlock(b: Pick<Block, 'author_type'>): boolean {
  // 判「不是参与者」而不是列举平台那一档的名字：这一列只分参与者和平台，而这份
  // bundle 会在已经打开的标签页里一直跑下去。列名字的话，往后哪一次平台那一档又
  // 换了名字，「<@bobby> 合并了 #12」就会掉进人说的话那一支，渲染成完整气泡加
  // 头像，一条谁都没说过的话。
