// 看板的呈现规则 —— **这里没有一行在算状态**。
//
// 它取代了原来的 `taskRing.ts`。那份文件从 status / residency / queued_at /
// accepted_at / card 五个事实里推出一条活处在哪一格，而后端现在也在推同一件事：
// 两个算法算同一个东西，迟早各说各的，而屏幕上那个词是哪一个算出来的，看的人分辨
// 不了。所以推导整个删掉了，只留下和后端无关的三件事：列怎么排、列叫什么、同一列
// 里谁排前面。
//
