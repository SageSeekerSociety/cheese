...
import type { FeedbackStatus, FeedbackTimelineEntry } from '@/lib/feedbackMock'
...
//
// 它画的是**梯子**（STATUS_LADDER），不是 timeline 数组本身 —— 数组里只有已经发生
// 过的几步，梯子是全部五步。少了这个区分，一条刚收录的反馈右侧只会有一个孤零零的
// 点，读的人看不出「后面还有几关」，而这正是这块要回答的问题。
//
...
const steps = computed<Step[]>(() => {
  const currentIndex = STATUS_LADDER.indexOf(props.status)
  return STATUS_LADDER.map((status, index) => {
    ...
    <li v-for="step in steps" :key="step.status" class="fb-step" :class="{ 'fb-step--current': step.current }">
      <span class="fb-step__rail" aria-hidden="true">
        <span
          ...
          :style="{ '--fb-dot': STATUS_META[step.status].dot }"
        />
      </span>
