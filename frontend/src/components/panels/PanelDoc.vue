...
function liveRefWidget(topicId: string): HTMLElement {
  ...
  el.title = t('workspace.doc.liveRefTitle', {
    title: subTitle ?? t('workspace.doc.thisTask'),
  ...
  label.textContent = subTitle ?? t('workspace.doc.subtopic')
...
// 留在这里的只有**拉取**，因为同一次请求还要喂编辑器里的评论下划线装饰 —— 拆开会
// 变成两次请求、两份可能不一致的真相。
...
function onDocClick(e: MouseEvent) {
  ...
  if (ca?.dataset.comment) {
    commentsRef.value?.locate(ca.dataset.comment)
    return
  }
...
async function commentOnSelection() {
  ...
  commentCta.value = null
  commentsRef.value?.open({ anchorId: anchor, quote: cta.quote })
}
