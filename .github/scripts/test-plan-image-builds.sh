...
assert_plan() {
  ...
  local actual
  actual="$(printf '%s\n' "$output" | grep -E '^(backend|sandbox|frontend|office_render|browser_render)=' | paste -sd, -)"
...
assert_plan 'backend=true,sandbox=true,frontend=true,office_render=true,browser_render=true' ''

commit_path frontend/src/main.ts
assert_plan 'backend=false,sandbox=false,frontend=true,office_render=false,browser_render=false' "$base_sha"

git -C "$test_repo" switch -q --detach "$base_sha"
commit_path backend/app/main.py
assert_plan 'backend=true,sandbox=false,frontend=false,office_render=false,browser_render=false' "$base_sha"

git -C "$test_repo" switch -q --detach "$base_sha"
commit_path backend/sandbox/cheese
assert_plan 'backend=true,sandbox=true,frontend=false,office_render=false,browser_render=false' "$base_sha"
...
assert_plan 'backend=false,sandbox=false,frontend=false,office_render=false,browser_render=true' "$base_sha"

git -C "$test_repo" switch -q --detach "$base_sha"
commit_path deploy/office-render/server.py
assert_plan 'backend=false,sandbox=false,frontend=false,office_render=true,browser_render=false' "$base_sha"

git -C "$test_repo" switch -q --detach "$base_sha"
commit_path docs/readme.md
assert_plan 'backend=false,sandbox=false,frontend=false,office_render=false,browser_render=false' "$base_sha"
assert_plan 'backend=true,sandbox=true,frontend=true,office_render=true,browser_render=true' "$base_sha" workflow_dispatch branch
assert_plan 'backend=true,sandbox=true,frontend=true,office_render=true,browser_render=true' "$base_sha" push tag
...
commit_path backend/app/main.py
commit_path frontend/src/main.ts
assert_plan 'backend=true,sandbox=false,frontend=true,office_render=false,browser_render=false' "$base_sha"
