...
- **Scoped per-turn token** (the default, minted by the compute provider for each
  turn): an HMAC over {project, topic, exp}. The gate verifies the signature AND
...
def mint_scoped_token(
    *, project_id: str, topic_id: str | None = None, ttl_s: int = _SCOPED_TTL_S
) -> str:
    """Mint an HMAC token scoped to a project (+ optional topic), expiring in ttl_s."""
    payload = {"p": project_id, "t": topic_id, "exp": int(time.time()) + ttl_s}
